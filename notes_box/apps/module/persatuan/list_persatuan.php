    ...
    <!--------------button action--------------->
              <div class="col-md-8 pull-left">
                  <div class="compose-btn">
                       <a class="btn btn-sm btn-primary" href="?module=add_persatuan"><i class="fa fa-plus"></i> Tambah persatuan</a>
    ...
    <!------------------content table------------>
                              ...
                              <table class="dataTables-example table table-striped responsive-utilities jambo_table bulk_action">
                                        <thead>
                                            <tr class="headings">
                                                ...
                                                <th class="column-title" width="20%">Nama</th>
                                                <th class="column-title" width="25%">Alamat</th>
                                                <th class="column-title" width="10%">Jumlah</th>
                                                <th class="column-title" width="15%">Tanggal </th>
                                                <th class="column-title" width="10%">Kembali </th>
                                                <th class="column-title" width="10%">Kategori</th>
                            ...
                            <tbody>
                                <?php
                                        ...
                                        $data = $query->show_persatuan();
                                        foreach ($data as $row){
                                            
                                echo"<tr class='even pointer'>
                                    <td class='a-center '><input type='checkbox' class='flat' name='table_records' ></td>
                                    <td class=' '>$row[nama_persatuan]</td>
                                    <td class=' '>$row[alamat_persatuan]</td>
                                    <td class=' '>Rp.$row[jumlah]</td>
                                    <td class=' '>".tgl_indo($row[tgl_persatuan])."</td>
                                    <td class=' '>Rp.$row[kembali]</td>
                                    <td class=' '>$row[kategori_persatuan]</td>
                                    <td class=' last'>
                                    <a href='?module=edit_persatuan&id=$row[id_persatuan]' title='Edit' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                                    <a href='?module=persatuan&act=hapus&id=$row[id_persatuan]' title='Hapus'  class='btn btn-sm btn-danger'><i class='fa fa-remove' onClick=\"return confirm('Apakah Anda benar-benar akan menghapus data ini ?')\"></i></a>
                                    </td>
                                </tr>";
                                }
                                ?>
...
if($_GET['act']=='hapus'){
    $id     = $_GET['id'];
    $query->delete_persatuan($id);
    $query->log_aktifitas($_SESSION[id_user],'Menghapus data persatuan',date('Y-m-d'),date('H:i:s'));
    echo "<script>window.location='?module=persatuan';</script>";
}
