                            ...
                            while (false !== ($file = readdir($handle))) {
                                    if ($file != "." && $file != "..") {
                                            ...
                                            if ($ekstensi!='' AND $ekstensi!='jpg' AND $filename!='index.html'){
                                                    echo "<li><a href='admin.php?mod_apps=themes&media_app=CSS&file=$filename'><i class='icon-file '></i>&nbsp;&nbsp;".str_replace(".css","",$filename)." <span class='pull-right'>".format_size(filesize(APP_CSS.$filename))."</span></a></li>";
                                            }else{ }
                                    }
                            }
                    }
    ...
    <div class="span11">
        <div class="blockoff-right">
            ...
            if(!empty($_GET['file'])) {
                ...
                echo "<h3><i class='icon-file'></i> &nbsp;File CSS : ".$_GET['file']."</h3>";
                echo "<p>Silahkan pilih baris yang akan diedit..</p>";
                echo "</div>";
                $filename = APP_CSS.validasi($_GET['file'], 'xss');
                $fh = fopen($filename, "r") or die("Could not open file!");
                $data = fread($fh, filesize($filename)) or die("Could not read file!");
                fclose($fh);
                echo "<form enctype='multipart/form-data' action='admin.php?mod_apps=themes&media_app=CSS&action=aksi_css' method='post'>";
                ...
                echo "<h3>Pilih CSS</h3>";
                echo "<p>Silahkan pilih file CSS yang akan di edit..</p>";
                echo "</div>";
            }
            ?>
        </div>
