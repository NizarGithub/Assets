    ...
    <!------------------content table------------>
                            ...
                            <tbody>
                                ...
                                echo"<tr class='even pointer'>
                                    ...
                                    <td class=' '>$row[keterangan_k_acount]</td>
                                    <td class=' last'>
                                    <a title='Edit' href='?module=kategori_account&act=edit&id=$row[id_k_acount]' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                                    <a href='?module=kategori_account&act=hapus&id=$row[id_k_acount]' title='Hapus'  class='btn btn-sm btn-danger'><i class='fa fa-remove' onClick=\"return confirm('Apakah Anda benar-benar akan menghapus akun $row[nama_acount] ?')\"></i></a>
                                    </td>
                                </tr>";
                                }
                                ?>
...
$data = $query->getone_kategori_acount($_GET['id']);
  ?>
                       <div class="col-md-5">  
                             <div class="product_price">
                                 <h4 class="pull-right" style="color:#26b99a;"><i class='fa fa-info'></i> edit</h4><br>   
                                 <form method="POST" action="?module=aksi_kategori_account" class="form-horizontal form-label-left">       
                                    <div class="form-group">
                                        <label>Nama Kategori</label>
                                        <input type="hidden" name="id" value="<?php echo $data[id_k_acount]; ?>">
                                        <input type="text" name="a" class="form-control" value="<?php echo $data[nama_k_acount]; ?>" placeholder="Nama Kategori">
                                    </div><br><br>
                                    <div class="form-group">
                                        <label>Keterangan</label>
                                        <textarea class="form-control" name="b" rows="3" placeholder='Keterangan'><?php echo $data[keterangan_k_acount]; ?></textarea>
                                    </div>
...
if($_GET['act']=='hapus'){
    $id     = $_GET['id'];
    $query->delete_kategori_acount($id);
    $query->log_aktifitas($_SESSION[id_user],'Menghapus data kategori box account',date('Y-m-d'),date('H:i:s'));
    echo "<script>window.location='?module=kategori_account';</script>";
}