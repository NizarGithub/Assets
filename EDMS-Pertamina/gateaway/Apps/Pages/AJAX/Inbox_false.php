...
$query = "SELECT * FROM inbox WHERE Processed='false' ORDER BY ID DESC";
$data  = $ARSql->query($query);
$nomor = 1;
while($rows = $ARSql->mf_object($data)) {
    echo "<tr class='error'><td>$nomor.</td>";
    echo "<td>$rows->SenderNumber</td>";
    echo "<td>".$rows->TextDecoded."</td>";
    echo "<td>Sekitar ".timeAgo(strtotime("$rows->ReceivingDateTime"))."</td>";
