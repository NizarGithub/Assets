                ...
                <div class="row-fluid">
                    <div class="span8 separate-sections">
                        ...
                        if (isset($_POST['save'])) {
                            ...
                            if (empty($file)) {
                                $query->tambah_advertisement($judul, $url, $file, $tgl_sekarang);
                                // Log Aktifitas
                                $query->log($_SESSION[id_user],'Menambahkan data advertisement',$tgl_sekarang,$jam_sekarang);
                                echo "<script>window.location='admin.php?page=advertisement';</script>";
                            } else {

                                // Query dengan melakukan upload gambar
                                if (move_uploaded_file($file, $path)) {
                                    $query->tambah_advertisement($judul, $url, $filename, $tgl_sekarang);
                                    // Log Aktifitas
                                    $query->log($_SESSION[id_user],'Menambahkan data advertisement',$tgl_sekarang,$jam_sekarang);
                                    echo "<script>window.location='admin.php?page=advertisement';</script>";
                                }
