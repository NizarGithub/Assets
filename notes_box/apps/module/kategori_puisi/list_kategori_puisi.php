    ...
    <!------------------content table------------>
                            ...
                            <tbody>
                                ...
                                echo"<tr class='even pointer'>
                                    ...
                                    <td class=' '>$row[ket_k_puisi]</td>
                                    <td class=' last'>
                                    <a title='Edit' href='?module=kategori_puisi&act=edit&id=$row[id_k_puisi]' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                                    <a href='?module=kategori_puisi&act=hapus&id=$row[id_k_puisi]' title='Hapus'  class='btn btn-sm btn-danger'><i class='fa fa-remove' onClick=\"return confirm('Apakah Anda benar-benar akan menghapusnya ?')\"></i></a>
                                    </td>
                                </tr>";
                                }
                                ?>
...
$data = $query->getone_kategori_puisi($_GET['id']);
  ?>
                       <div class="col-md-5">  
                             <div class="product_price">
                                 <h4 class="pull-right" style="color:#26b99a;"><i class='fa fa-info'></i> edit</h4><br>   
                                 <form method="POST" action="?module=aksi_kategori_puisi" class="form-horizontal form-label-left">       
                                    <div class="form-group">
                                        <label>Nama Kategori</label>
                                        <input type="hidden" name="id" value="<?php echo $data[id_k_puisi]; ?>">
                                        <input type="text" name="a" class="form-control" value="<?php echo $data[nama_k_puisi]; ?>" placeholder="Nama Kategori">
                                    </div><br><br>
                                    <div class="form-group">
                                        <label>Keterangan</label>
                                        <textarea class="form-control" name="b" rows="3" placeholder='Keterangan'><?php echo $data[ket_k_puisi]; ?></textarea>
                                    </div>
...
if($_GET['act']=='hapus'){
    $id     = $_GET['id'];
    $query->delete_kategori_puisi($id);
    $query->log_aktifitas($_SESSION[id_user],'Menghapus data kategori puisi',date('Y-m-d'),date('H:i:s'));
    echo "<script>window.location='?module=kategori_puisi';</script>";
}