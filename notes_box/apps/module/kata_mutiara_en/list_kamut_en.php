    ...
    <!--------------button action--------------->
              <div class="col-md-8 pull-left">
                  <div class="compose-btn">
                       <a class="btn btn-sm btn-primary" href="?module=add_kamut_english"><i class="fa fa-plus"></i> Tambah Kata Mutiara (en)</a>
    ...
    <!------------------content table------------>
                              ...
                              <table class="dataTables-example table table-striped responsive-utilities jambo_table bulk_action">
                                        <thead>
                                            <tr class="headings">
                                                ...
                                                <th class="column-title" width="30%">Kata Mutiara </th>
                                                <th class="column-title" width="30%">Arti </th>
                                                <th class="column-title" width="20%">Pengutip </th>
                            ...
                            <tbody>
                                <?php
                                        ...
                                        $data = $query->show_kamut_en();
                                        foreach ($data as $row){
                                            
                                echo"<tr class='even pointer'>
                                    <td class='a-center '><input type='checkbox' class='flat' name='table_records' ></td>
                                    <td class=' '>$row[kamut_en]</td>
                                    <td class=' '>$row[arti]</td>
                                    <td class=' '>$row[pengutip]</td>
                                    <td class=' '>$row[nama_k_kamut]</td>
                                    <td class=' last'>
                                    <a href='?module=edit_kamut_english&id=$row[id_kamut_en]' title='Edit' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                                    <a href='?module=kamut_english&act=hapus&id=$row[id_kamut_en]' title='Hapus'  class='btn btn-sm btn-danger'><i class='fa fa-remove' onClick=\"return confirm('Apakah Anda benar-benar akan menghapus data ini ?')\"></i></a>
                                    </td>
                                </tr>";
                                }
                                ?>
...
if($_GET['act']=='hapus'){
    $id     = $_GET['id'];
    $query->delete_kamut_en($id);
    $query->log_aktifitas($_SESSION[id_user],'Menghapus data kata mutiara (english)',date('Y-m-d'),date('H:i:s'));
    echo "<script>window.location='?module=kamut_english';</script>";
}